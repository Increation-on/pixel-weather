...
export const CityItem: React.FC<CityItemProps> = ({ item, onSelect }) => {
  ...
  return (
    ...
    >
      <Text className="text-white font-pixel text-base mb-1">
        {item.city || 'Неизвестный город'}
      </Text>
      <Text className="text-gray-400 font-pixel text-sm mb-1">
        {item.country && `${item.country} • `}
        {getTypeText(item.type)}
      </Text>
      <Text className="text-gray-500 font-pixel text-xs">
